...
app.directive('headerLanding',function(){
	return {
		restrict: 'E',
		controller:'headerCtrl',
		templateUrl:'Front-End/app/pages/header-home/headerLanding.html',
		link: function(scope, element, attributes){
		}
	};
});
...
app.directive('userProfile',function(){
	return {
		restrict: 'E',
		controller:'headerCtrl',
		templateUrl:'Front-End/app/pages/header-home/user_profile.html',
		link: function(scope, element, attributes){
		}
	};
});
...
app.controller('headerCtrl',['$scope','$rootScope','$location','$window','$localStorage','appDB','$filter','Upload','environment','$http','$interval',function($scope,$rootScope,$location,$window,$localStorage,appDB,$filter,Upload,environment,$http,$interval){
		...
		.then(
			function success(data){ 
				...
				if(data.code == 200 && data.status == 1){ 
					...
					$scope.dob=data.responsedate_of_birth;
	...
	$scope.updateProfile= function (form) {
		var data={};
		var dob= $filter('date')(new Date($scope.users.date_of_birth), "yyyy-MM-dd"); 
   ...
   // }
	...
	$scope.liveScoreDetails = function(){
		$scope.liveScoreData = {};
		var $data = {};
		$data.login_session_key = $localStorage.userDetails.login_session_key;
		appDB
		.callPostForm('match/matches_live_score',$data) 
		.then(
			function success(data){ 
				if(data.code == 200 && data.status == 1){ 
					$scope.liveScoreData=data.response;
					console.log('----',$scope.liveScoreData);
					// $scope.seriesErrMsg="";
				} 
			},
			function error(data){
